...
class OntopProcessManager:
    ...
    def start(self) -> None:
        ...
        # ~80s on a 48-TriplesMap mapping. That can exceed the reformulate client
        # timeout and surface as a misleading "Failed to reach Ontop reformulate
        # endpoint" 502. Initialising eagerly moves the cost into startup, so the port
        # only opens once Ontop can actually serve. See Ontop's
        ...
        # mapping needs (~78s measured for 48 TriplesMaps). Allow generous headroom for
